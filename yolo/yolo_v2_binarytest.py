...
class yolo_v2(object):
    ...
    def conv_layer(self, inputs, shape, batch_norm = True, name = '0_conv'):
        ...
        return conv
#check code for binary kernel
    '''def conv_layer_binary(self, inputs, shape, batch_norm = True, name = '0_conv'):
        self.kernel = self.add_weight(name="'kernel'", shape, trainable=True, initializer)
        binary_kernel = kernel + K.stop_gradient(K.sign(kernel) - kernel)
